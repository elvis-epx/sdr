...
for pos in range(0, len(original), FFT_SAMPLE):
   ...
   # mask positive frequencies (f[0] is DC component)
    ...
    #time_domain = [ bound(np.int(real(sample))) * 32767 for sample in time_domain ]
    #print(type(time_domain[i]))
    time_domain = [ int(bound(real(sample)) * 32767) for sample in time_domain ]
    #print((time_domain))
    #print((time_domain[i]))
